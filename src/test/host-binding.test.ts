...
async function testHostBinding() {
  ...
  // Start the server
  const child = spawn("node", ["dist/index.js", "--transport", "http", "--port", testPort.toString()], {
  ...
  for (const testCase of testCases) {
    try {
      ...
      console.log(`❌ ${testCase.description} - Connection failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
